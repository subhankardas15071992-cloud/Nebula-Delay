...
fn draw_nebula_editor(ui: &mut Ui, state: &mut EditorState, setter: &ParamSetter<'_>) {
    ...
    painter.text(
        c.pos(48.0, 48.0),
        Align2::LEFT_CENTER,
        "Mono Delay Processor  |  Native UI  |  64-bit",
        c.font(11.0),
        TEXT_SEC,
    );
    painter.text(
    ...
    draw_nebula_mono_channel(ui, state, setter, c, LEFT_X);
...
fn draw_nebula_level_meter(
    ...
    let (level, label, slider_id) = if is_input {
        (state.meters.get_input_l(), "INPUT", "input_level_meter")
    } else {
    ...
    draw_nebula_meter_track(&painter, c, meter, level);
...
fn draw_nebula_mono_channel(
    ...
    nebula_phase_button(
        ...
        setter,
        c,
        c.rect(panel_cx - 40.0, 540.0, 80.0, 24.0),
        &params.feedback_phase_l,
        "PHASE",
    );
}

fn draw_nebula_mono_delay(
    ui: &mut Ui,
    state: &mut EditorState,
    setter: &ParamSetter<'_>,
    c: LogicCanvas,
...
fn param_id_for(name: &str) -> String {
    match name {
        "Delay Time" => "delay_time_l".to_string(),
        "Note" => "note_l".to_string(),
        "Halve" => "halve_l".to_string(),
        "Double" => "double_l".to_string(),
        "HPF" => "low_cut_l".to_string(),
        "HPFS" => "low_cut_slope_l".to_string(),
        "LPF" => "high_cut_l".to_string(),
        "LPFS" => "high_cut_slope_l".to_string(),
        "Feedback" => "feedback_l".to_string(),
        "Phase Invert" => "feedback_phase_l".to_string(),
        "Dry Level" => "output_mix_l".to_string(),
        "Wet Level" => "output_mix_r".to_string(),
        "Mix" => "output_mix_l".to_string(),
