...
//!
//! ```text
//! Input ──► Hard Bypass Gate ──► Input Mode Selection ──────────────► Dry/Wet Levels ──► Output
//!                                      │                                  ▲
//!                                      ▼                                  │
...
pub struct DelayParams {
    ...
    // ── Output ────────────────────────────────────────────────────────
    /// Dry signal level for the left channel.
    pub output_mix_l: f64,
    /// Wet signal level for the left channel in the mono plugin, and the wet
    /// level for the right lane when the shared DSP engine is tested directly.
    pub output_mix_r: f64,

    /// Invert the left delayed signal at the dry/wet mix stage.
    pub wet_phase_l: bool,
    /// Invert the right delayed signal at the dry/wet mix stage.
    pub wet_phase_r: bool,
...
impl DelayEngine {
    ...
    pub fn process_frame(
        ...
        // ── 12. Output routing ────────────────────────────────────────
        let (mut wet_l, mut wet_r) = Self::apply_output_routing(p.routing, filt_l, filt_r);
        if p.wet_phase_l {
            wet_l = -wet_l;
        }
        if p.wet_phase_r {
        ...
        // ── 13. Dry/wet levels ────────────────────────────────────────
        //   output_mix_l is the dry level, output_mix_r is the wet level.
        let out_l = in_l * s_mix_l + wet_l * s_mix_r;
        let out_r = in_r * s_mix_l + wet_r * s_mix_r;
