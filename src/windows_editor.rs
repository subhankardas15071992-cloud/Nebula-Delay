...
impl Editor for NativeEditor {
    fn spawn(
        ...
        let desired_w = (user_w * render_scale).round() as i32;
        let desired_h = (user_h * render_scale).round() as i32;
...
impl NativeWindowState {
    ...
    fn draw_background(
        ...
        draw_text(
            rt,
            "Mono Delay  |  Native Direct2D  |  VST3",
            UiRect::new(
                18.0 * layout.s,
                34.0 * layout.s,
                275.0 * layout.s,
    ...
    fn draw_channel(
        ...
        draw_button(
            rt,
            panel.deviation,
            if tempo_sync { "Sync On" } else { "Sync Off" },
            tempo_sync,
            brushes,
            formats,
            s,
        ...
        let dry = FloatControl::OutputMixL;
        let wet = FloatControl::OutputMixR;
        let dry_cx = panel.panel.center_x();
        let wet_cx = panel.crossfeed_cx;
        draw_text(
            rt,
            "Feedback",
        ...
        draw_value_box(
            ...
            &format_float(self.float_param(dry)),
            brushes,
            formats,
        );
        ...
        draw_value_box(
            ...
            &format_float(self.float_param(wet)),
            brushes,
            formats,
        );
    ...
    fn hit_zones(&self, layout: &Layout) -> Vec<HitZone> {
        ...
        let tempo_sync = self.params.tempo_sync.value();
        mono_channel_zones(&mut zones, &layout.left, tempo_sync);
        zones
    }
    ...
    fn dropdown_anchor(&self, control: EnumControl, layout: &Layout) -> UiRect {
        match control {
            ...
            EnumControl::Oversampling => layout.left.input,
        }
    }
    ...
    fn value_rect(&self, control: FloatControl, layout: &Layout) -> Option<UiRect> {
        value_rect_for_control(control, &layout.left, Channel::Left).or_else(|| match control {
            ...
            FloatControl::OutputMixL => Some(UiRect::new(
                layout.left.panel.center_x() - 44.0 * layout.s,
                layout.left.crossfeed_cy + 39.0 * layout.s,
                88.0 * layout.s,
                18.0 * layout.s,
            )),
            FloatControl::OutputMixR => Some(UiRect::new(
                layout.left.crossfeed_cx - 44.0 * layout.s,
                layout.left.crossfeed_cy + 39.0 * layout.s,
                88.0 * layout.s,
                18.0 * layout.s,
            )),
...
impl ChannelLayout {
    fn new(panel: UiRect, s: f32) -> Self {
        ...
        Self {
            ...
            feedback_cx,
            feedback_cy: row_y,
            feedback_phase: UiRect::new(cx - 40.0 * s, row_y + 66.0 * s, 80.0 * s, 22.0 * s),
...
fn mono_channel_zones(zones: &mut Vec<HitZone>, layout: &ChannelLayout, tempo_sync: bool) {
    zones.push(HitZone::new(
        layout.input,
    ...
    if tempo_sync {
        zones.push(HitZone::new(
            layout.note,
            Action::Dropdown(EnumControl::NoteL),
        ));
    }
    zones.push(HitZone::new(
        layout.halve,
    ...
    zones.push(HitZone::new(
        ...
        Action::Float(FloatControl::DelayTimeL),
    ));
    ...
    zones.push(HitZone::new(
        knob_rect(layout.feedback_cx, layout.feedback_cy, 43.0 * layout.s),
        Action::Float(FloatControl::FeedbackL),
    ));
    zones.push(HitZone::new(
        UiRect::new(
            layout.feedback_cx - 44.0 * layout.s,
        ...
        Action::FloatField(FloatControl::FeedbackL),
    ));
    zones.push(HitZone::new(
        layout.feedback_phase,
        Action::Bool(BoolControl::FeedbackPhaseL),
    ));
    let dry_cx = layout.panel.center_x();
    zones.push(HitZone::new(
        knob_rect(dry_cx, layout.crossfeed_cy, 43.0 * layout.s),
        Action::Float(FloatControl::OutputMixL),
    ));
    zones.push(HitZone::new(
        UiRect::new(
            dry_cx - 44.0 * layout.s,
            layout.crossfeed_cy + 39.0 * layout.s,
            88.0 * layout.s,
            18.0 * layout.s,
        ),
        ...
        match msg {
            ...
            WM_MOUSEMOVE => {
                ...
                if state.drag.is_some() || state.resize_drag.is_some() {
                    ...
                    LRESULT(0)
                } else if resize_grip_rect(&state.current_layout()).contains(x, y) {
